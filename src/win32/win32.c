...
static LRESULT __stdcall window_proc(HWND hwnd, unsigned int msg, WPARAM wParam,
                                     LPARAM lParam) {
    switch(msg) {
        case WM_CLOSE:
            DestroyWindow(hwnd);
...
static bool register_window_class() {
    ...
    if(!RegisterClassEx(&wc)) {
        ...
        if(err == ERROR_CLASS_ALREADY_EXISTS)
            ...
            autoaway_log(
                "Error: Unable to register the window class due to an unknown "
                "reason");
        return false;
    }
    return true;
...
static unsigned long __stdcall thread_proc(void * param) {
    ...
    if(registered) {
        window_handle = CreateWindowEx(
            0, WINDOW_CLASS, WINDOW_TITLE, WS_EX_PALETTEWINDOW, CW_USEDEFAULT,
            CW_USEDEFAULT, 0, 0, HWND_MESSAGE, NULL, module_handle, NULL);
